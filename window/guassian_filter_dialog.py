...
class GuassianFilterDialog(wx.Dialog):
    ...
    def __init_widgets(self):
        ...
        stext_sz = wx.StaticText(panel, -1, "Sigma")
        sz_layout = wx.BoxSizer(wx.HORIZONTAL)
        self.slider_size = wx.Slider(panel, -1, minValue=0, maxValue=10.0,)
    ...
    def __on_text_ctrl_sz_changed(self, evt):
        ...
        self.slider_size.SetValue(float(label))

    def __on_btn_confirm_click(self, evt):
        
        sz = self.text_ctrl_size.GetLineText(0)
        self.function.gaussian_blur(float(sz))
